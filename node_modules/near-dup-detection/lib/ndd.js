...
// it always can be better
    textCanonization : function(text, callback) {
...
// function for clearing the text from all special characters
  ...
  makeShingles : function(text, callback) {
  ...
  hashingShingles : function(shingles, callback) {
    ...
    compareShingles : function(arr1, arr2, callback) {
	      var count = 0;
	      ...
	      callback(null, count*2/(arr1[0].length + arr2[0].length));
    },

    getShingles : function(content1, callback){
	      Ndd.textCanonization(content1, function(text) {
	        Ndd.makeShingles(text, function(shingles) {
	          Ndd.hashingShingles(shingles, function(hashes) {
...
//  var content2 = fileJSON2.content;
    ...
    compareTexts : function(content1, content2, callback){
	  async.parallel([
	    function(callback){
	      this.textCanonization(content1, function(text) {
	        this.makeShingles(text, function(shingles) {
	          this.hashingShingles(shingles, function(hashes) {
	            callback(null, hashes);
	          });
	        })
	      });
	    ...
	    function(callback){
	      this.textCanonization(content2, function(text) {
	        this.makeShingles(text, function(shingles) {
	          this.hashingShingles(shingles, function(hashes) {
	            callback(null, hashes);
	          });
	    ...
	    this.compareShingles(firstHashes, secondHashes, function(err, res){
		    callback(null, res);
	    });
